...
"timeupdate",function(a){b.updatePlayhead.apply(b)});f[g].events.addListener(b.element,"ended",function(a){b.trackEnded.apply(b)});f[g].events.addListener(b.source,"error",function(a){clearInterval(b.readyTimer);clearInterval(b.loadTimer);b.settings.loadError.apply(b)})}},helpers:{merge:function(a,b){for(attr in b)if(a.hasOwnProperty(attr)||b.hasOwnProperty(attr))a[attr]=b[attr]},clone:function(a){function b(a){if(null==a||"object"!==typeof a)return a;var c=new a.constructor,e;for(e in a)c[e]=b(a[e]);
return c}return b(a)},addClass:function(a,b){(new RegExp("(\\s|^)"+b+"(\\s|$)")).test(a.className)||(a.className+=" "+b)},removeClass:function(a,b){a.className=a.className.replace(new RegExp("(\\s|^)"+b+"(\\s|$)")," ")},injectCss:function(a,b){var c="",d=document.getElementsByTagName("style");b=b.replace(/\$1/g,a.settings.imageLocation);b=b.replace(/\$2/g,a.settings.retinaImageLocation);a=0;for(var e=d.length;a<e;a++){var f=d[a].getAttribute("title");if(f&&~f.indexOf("audiojs")){e=d[a];if(e.innerHTML===
...
return c.firstChild}},events:{memoryLeaking:!1,listeners:[],addListener:function(a,b,c){a.addEventListener?a.addEventListener(b,c,!1):a.attachEvent&&(this.listeners.push(a),this.memoryLeaking||(window.attachEvent("onunload",function(){if(this.listeners)for(var a=0,b=this.listeners.length;a<b;a++)f[g].events.purge(this.listeners[a])}),this.memoryLeaking=!0),a.attachEvent("on"+b,function(){c.call(a,window.event)}))},trackLoadProgress:function(a){if(a.settings.preload){var b,c=/(ipod|iphone|ipad)/i.test(navigator.userAgent);
